...
In the derivation above, we use the following compact notations: the training data is represented by the $N*\left(p+1\right)$ matrix $\mathbf{X}$, the true responses by $\mathbf{y} = \left(y_1, y_2, \dots, y_N\right)^T$, and the noises by $\mathbf{\epsilon} = \left(\epsilon_1, \epsilon_2, \dots, \epsilon_N\right)^T$.
